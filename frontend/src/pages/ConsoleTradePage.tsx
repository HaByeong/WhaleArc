...
import { tradeService, type Portfolio, type Trade, type Holding, type Order, type PriceAlert } from '../services/tradeService';
  ...
  <aside style={{ ...mkCard, padding: 0, display: 'flex', flexDirection: 'column', minHeight: 820 }}>
    <div className="flex gap-1 p-3 pb-0">
      {ASSET_CLASSES.map(({ key, label }) => (
        <button key={key} onClick={() => setKlass(key)} className="flex-1 whitespace-nowrap rounded-lg px-1.5 py-2 text-[13.5px] font-semibold" style={{ border: klass === key ? '1px solid rgba(91,157,255,.32)' : `1px solid ${LINE}`, background: klass === key ? 'rgba(91,157,255,.12)' : 'transparent', color: klass === key ? 'var(--ci-ink0)' : INK1 }}>{label}</button>
      ))}
    </div>
    <div className="relative px-3.5 pb-3 pt-3" style={{ borderBottom: `1px solid ${LINE}` }}>
      <div className="relative">
        ...
        <input value={query} onChange={e => setQuery(e.target.value)} aria-label="종목 검색" placeholder={canSearch ? '종목 검색 (예: 삼성, AAPL)' : '코인 검색…'} className="w-full rounded-lg py-2.5 pl-[34px] pr-3 text-[14px] outline-none" style={{ border: `1px solid ${LINE}`, background: 'var(--ci-card)', color: 'var(--ci-ink0)' }} />
      </div>
      {canSearch && searchResults.length > 0 && (
        <div className="no-scrollbar absolute left-3.5 right-3.5 top-[60px] z-10 max-h-[260px] overflow-y-auto rounded-lg p-1" style={{ border: `1px solid ${LINE}`, background: 'var(--ci-raised)', boxShadow: '0 12px 30px -10px rgba(0,0,0,.5)' }}>
          {searchResults.map(r => <button key={r.code} onClick={() => onSearchPick(r)} className="flex w-full items-center justify-between rounded px-2.5 py-1.5 text-left text-[13.5px] hover:bg-white/5"><span className="font-semibold">{r.name}</span><span className="font-mono" style={{ color: INK3 }}>{r.code}</span></button>)}
        </div>
      )}
      <div className="mt-3 flex items-center justify-between">
        <span className="text-[12.5px] tracking-[.06em]" style={{ color: INK2 }}>{(ASSET_CLASSES.find(c => c.key === klass) || ASSET_CLASSES[0]).label} <span className="font-mono font-semibold" style={{ color: 'var(--ci-ink0)' }}>{assets.length}개</span></span>
      </div>
    </div>
    <ul className="no-scrollbar m-0 flex-1 list-none overflow-y-auto p-0" style={{ maxHeight: 720 }}>
      {loading && assets.length === 0 && <li className="px-3.5 py-6 text-center text-[13.5px]" style={{ color: INK3 }}>불러오는 중…</li>}
      {!loading && assets.length === 0 && <li className="px-3.5 py-6 text-center text-[13.5px]" style={{ color: INK3 }}>표시할 종목이 없습니다</li>}
      {assets.map((s, i) => {
        ...
        return (
          <li key={s.symbol}>
            <button onClick={() => onPick(s.symbol)} className="grid w-full grid-cols-[1fr_auto] items-center gap-2.5 px-3.5 py-3 text-left" style={{ background: isActive ? 'rgba(91,157,255,.10)' : 'transparent', borderLeft: isActive ? `2px solid ${GLOW}` : '2px solid transparent', borderBottom: i === assets.length - 1 ? 'none' : `1px solid ${LINE}`, color: 'var(--ci-ink0)' }}>
              <div className="flex min-w-0 flex-col gap-0.5"><span className="truncate text-[14.5px]" style={{ fontWeight: isActive ? 700 : 600 }}>{s.name}</span><span className="font-mono text-[12px]" style={{ color: INK3 }}>{s.symbol}</span></div>
              <div className="flex flex-col gap-0.5 text-right"><span className="font-mono text-[14px] font-semibold">{fmtPrice(s, usdKrw)}</span><span className="font-mono text-[12.5px] font-semibold" style={{ color: up ? UP : DOWN }}>{up ? '+' : ''}{s.changeRate.toFixed(2)}%</span></div>
            </button>
          </li>
        );
      })}
...
const InlineStat = ({ label, value }: { label: string; value: string }) => (
  <div className="flex min-w-0 flex-col gap-1"><span className="text-[11.5px] font-semibold tracking-[.12em]" style={{ color: INK2 }}>{label}</span><span className="truncate font-mono text-[16px] font-semibold">{value}</span></div>
);
      ...
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div className="min-w-0">
          <div className="mb-2 flex items-center gap-2.5">
            <span className="rounded-[5px] px-2 py-[3px] text-[11.5px] font-bold tracking-[.08em]" style={{ background: 'rgba(91,157,255,.14)', color: '#cfe1ff', border: '1px solid rgba(91,157,255,.24)' }}>{sel.market || '—'}</span>
            <span className="font-mono text-[13px]" style={{ color: INK3 }}>{sel.symbol}</span>
          </div>
          <h2 className="text-[28px] font-bold tracking-tight">{sel.name}</h2>
        </div>
        <div className="flex flex-col gap-1.5 text-right">
          <span className="font-mono text-[34.5px] font-semibold tracking-tight">{fmtPrice(sel, usdKrw)}</span>
          <span className="font-mono text-[15px] font-semibold" style={{ color: up ? UP : DOWN }}>{up ? '+' : ''}{(fx ? sel.change * usdKrw : sel.change).toLocaleString('ko-KR', { maximumFractionDigits: 0 })} ({up ? '+' : ''}{sel.changeRate.toFixed(2)}%)</span>
        </div>
      </div>
...
const NumberField = ({ value, onChange, step = 1, suffix, disabled, comma }: { value: number; onChange: (n: number) => void; step?: number; suffix?: string; disabled?: boolean; comma?: boolean }) => {
  // comma=true면 천 단위 콤마 표시(소수부 보존). 편집 중에는 text 버퍼로 '150.'·'0.0012' 같은 입력 중 소수점을 유지하고,
  // 블러/스테퍼 시 null로 비워 숫자값(value)에서 다시 포맷한다.
...
const OrderTicket = ({ sel, side, setSide, portfolio, usdKrw, rtPrice, notify, onPlaced, isVirt }: {
  ...
  // 지정가 가격 단계 — 크립토는 저가 알트코인까지 고려해 가격대별로 세분화(Orderbook 호가 단계와 동일 결).
  ...
  // 수량 스텝 — 주식/ETF는 1주, 크립토는 0.0001개 단위 미세 조정 허용(소수 8자리 매매 지원)
...
const ChartPanel = ({ symbol, price, changeRate, assetType, indicators, setIndicators }: { symbol: string; price: number; changeRate: number; assetType: AssetType; indicators: Indicators; setIndicators: (fn: (x: Indicators) => Indicators) => void }) => (
  <div className="px-5 py-[18px]">
    ...
    {indicators.ma && <div className="mb-2 flex flex-wrap gap-3 text-[11.5px]" style={{ color: INK2 }}>{MA_DEFS.map(([p, c]) => <span key={p} className="inline-flex items-center gap-1.5"><span style={{ width: 12, height: 2, background: c, display: 'inline-block' }} />MA{p}</span>)}</div>}
...
const FillsPanel = ({ trades, usdKrw }: { trades: Trade[]; usdKrw: number }) => (
  <div className="px-5 py-[18px]">
    {trades.length === 0 ? <div className="py-10 text-center text-[14px]" style={{ color: INK3 }}>체결 내역이 없습니다.</div> : (
      <table className="w-full border-collapse"><thead><tr>{['시간', '구분', '수량', '체결가', '체결액', '모드'].map(h => <th key={h} style={thSx}>{h}</th>)}</tr></thead>
        <tbody>{trades.map(t => { const buy = t.orderType === 'BUY', stockLike = t.assetType === 'STOCK' || isUsdAsset(t.assetType); return (<Fragment key={t.id}><tr>
          ...
          <td style={t.memo ? { ...tdSx, borderBottom: 'none' } : tdSx}><span className="rounded-[5px] px-2 py-0.5 text-[12px] font-bold" style={{ color: buy ? UP : DOWN, background: buy ? 'rgba(239,77,77,.12)' : 'rgba(77,138,255,.12)' }}>{buy ? '매수' : '매도'}</span></td>
          ...
          <td style={t.memo ? { ...tdSx, borderBottom: 'none' } : tdSx}><span className="rounded px-1.5 py-0.5 text-[11px] font-bold tracking-[.06em]" style={{ background: 'rgba(180,210,255,.18)', color: '#cfe1ff' }}>VIRT</span></td>
        </tr>{t.memo && <tr><td colSpan={6} style={{ padding: '0 12px 12px', borderBottom: `1px solid ${LINE}` }}><span className="text-[13px] italic" style={{ color: INK3 }}>📝 {t.memo}</span></td></tr>}</Fragment>); })}</tbody></table>
    )}
  </div>
);
...
const OrderHistoryPanel = ({ orders, onCancel }: { orders: Order[]; onCancel: (id: string) => void }) => {
  ...
  return (
    <div className="px-5 py-[18px]">
      {sorted.length === 0 ? <div className="py-10 text-center text-[14px]" style={{ color: INK3 }}>주문 내역이 없습니다.</div> : (
        <table className="w-full border-collapse"><thead><tr>{['시간', '구분', '종목', '수량', '가격', '방식', '상태'].map(h => <th key={h} style={thSx}>{h}</th>)}</tr></thead>
          <tbody>{sorted.map(o => { const buy = o.orderType === 'BUY', st = ORDER_STATUS[o.status] || ORDER_STATUS.PENDING, stockLike = o.assetType === 'STOCK' || isUsdAsset(o.assetType); return (
            <tr key={o.id}>
              ...
              <td style={tdSx}><span className="rounded-[5px] px-2 py-0.5 text-[12px] font-bold" style={{ color: buy ? UP : DOWN, background: buy ? 'rgba(239,77,77,.12)' : 'rgba(77,138,255,.12)' }}>{buy ? '매수' : '매도'}</span></td>
              <td style={tdSx}><div className="flex flex-col gap-0.5"><span className="font-semibold">{o.stockName}</span><span className="font-mono text-[12px]" style={{ color: INK3 }}>{o.stockCode}</span></div></td>
              ...
              <td style={tdSx}><span className="text-[13px]" style={{ color: INK2 }}>{o.orderMethod === 'MARKET' ? '시장가' : '지정가'}</span></td>
              <td style={tdSx}><div className="flex items-center gap-2"><span className="rounded px-1.5 py-0.5 text-[11.5px] font-bold" style={{ color: st.color, background: st.bg }}>{st.label}</span>{o.status === 'PENDING' && <button onClick={() => onCancel(o.id)} className="rounded-md px-2 py-0.5 text-[12px] font-semibold" style={{ border: `1px solid ${LINE}`, color: INK1 }}>취소</button>}</div></td>
            </tr>
          ); })}</tbody></table>
      )}
    </div>
...
const HoldingsPanel = ({ holdings, usdKrw, onPick }: { holdings: Holding[]; usdKrw: number; onPick: (sym: string, at?: string) => void }) => (
  <div className="px-5 py-[18px]">
    {holdings.length === 0 ? <div className="py-10 text-center text-[14px]" style={{ color: INK3 }}>보유 종목이 없습니다.</div> : (
      <table className="w-full border-collapse"><thead><tr>{['종목', '수량', '평균가', '현재가', '평가액', '평가 손익', '수익률'].map(h => <th key={h} style={thSx}>{h}</th>)}</tr></thead>
        <tbody>{holdings.map(h => { const up = h.profitLoss >= 0, stockLike = h.assetType === 'STOCK' || isUsdAsset(h.assetType); return (
          <tr key={h.stockCode} onClick={() => onPick(h.stockCode, h.assetType)} className="cursor-pointer transition-colors hover:bg-white/5">
            <td style={tdSx}><div className="flex flex-col gap-0.5"><span className="font-semibold">{h.stockName}</span><span className="font-mono text-[12px]" style={{ color: INK3 }}>{h.stockCode}</span></div></td>
...
const VirtPromo = ({ onGo }: { onGo: () => void }) => (
  <article className="relative overflow-hidden rounded-2xl px-[22px] py-6" style={{ background: 'linear-gradient(160deg, rgba(91,157,255,.18), rgba(91,157,255,.04) 50%, transparent)', border: '1px solid rgba(91,157,255,.30)' }}>
    ...
    <div className="mb-2.5 flex items-center justify-center gap-2"><span className="rounded px-1.5 py-0.5 text-[11px] font-bold tracking-[.06em] text-white" style={{ background: GLOW }}>VIRT</span><span className="text-[12.5px] font-semibold tracking-[.12em]" style={{ color: '#9cc1ff' }}>가상 거래</span></div>
    <h3 className="mb-2 text-center text-[18.5px] font-bold tracking-tight">Virt에서 안전하게 거래하세요</h3>
    <p className="m-0 text-center text-[14px] leading-snug" style={{ color: INK1 }}>가상돈으로 매수·매도 주문을 체험하고<br />전략을 테스트해보세요.</p>
    <button onClick={onGo} className="mt-4 w-full rounded-[10px] px-4 py-3 text-[14.5px] font-semibold text-white" style={{ background: `linear-gradient(180deg, ${GLOW}, ${ACCENT})`, boxShadow: '0 10px 24px -10px rgba(60,120,255,.6)' }}>Virt에서 거래하기 →</button>
  </article>
);
const PortfolioRow = ({ label, value, emphasis }: { label: string; value: ReactNode; emphasis?: boolean }) => (
  <div className="flex items-baseline justify-between"><dt className="text-[14px]" style={{ color: INK2 }}>{label}</dt><dd className="m-0 font-mono" style={{ fontSize: emphasis ? 18 : 14, fontWeight: emphasis ? 700 : 600 }}>{value}</dd></div>
);
const PortfolioMini = ({ portfolio, onGo, onPick }: { portfolio: Portfolio | null; onGo: () => void; onPick: (sym: string, at?: string) => void }) => {
  ...
  return (
    <article style={{ ...mkCard, padding: '22px 22px' }}>
      <div className="mb-[18px] flex items-baseline justify-between"><h3 className="text-[15px] font-bold">내 포트폴리오</h3><button onClick={onGo} className="text-[12px]" style={{ color: INK2 }}>전체 →</button></div>
      <dl className="m-0 flex flex-col gap-3">
        ...
        <PortfolioRow label="평가 손익" value={portfolio ? <span style={{ color: up ? UP : DOWN }}>{up ? '▲ +' : '▼ '}{rate.toFixed(2)}% <span className="text-[12.5px]" style={{ color: INK2 }}>({up ? '+' : ''}{fmtKRW(pnl)})</span></span> : '—'} />
      ...
      <div className="mt-[18px] pt-4" style={{ borderTop: `1px solid ${LINE}` }}>
        <div className="mb-2.5 text-[12px] font-semibold tracking-[.12em]" style={{ color: INK3 }}>보유 종목</div>
        {top.length === 0 ? <div className="text-[13.5px]" style={{ color: INK3 }}>보유 종목이 없습니다.</div> : (
          <ul className="m-0 flex list-none flex-col gap-2.5 p-0">{top.map(h => { const hu = h.returnRate >= 0; return <li key={h.stockCode}><button onClick={() => onPick(h.stockCode, h.assetType)} className="flex w-full items-center justify-between text-left text-[14px] transition-colors hover:opacity-80"><span className="truncate" style={{ color: INK1 }}>{h.stockName}</span><span className="font-mono font-semibold" style={{ color: hu ? UP : DOWN }}>{hu ? '+' : ''}{h.returnRate.toFixed(1)}%</span></button></li>; })}</ul>
        )}
      </div>
    </article>
  );
...
const PendingOrders = ({ orders, onCancel }: { orders: Order[]; onCancel: (id: string) => void }) => {
  ...
  return (
    <div className="px-5 pb-5" style={{ borderTop: `1px solid ${LINE}`, paddingTop: 18 }}>
      <div className="mb-2.5 text-[13px] font-semibold tracking-[.08em]" style={{ color: INK2 }}>미체결 주문 <span className="font-mono" style={{ color: GLOW }}>{open.length}</span></div>
  ...
  return (
    <HelmShell active="trade" virt={isVirt} userName={userName} session={assetType === 'CRYPTO' ? '실시간 시세 · WebSocket' : '시세 10초 갱신'}>
      <div className="flex flex-col gap-6">
        <div>
          <div className="mb-3 flex flex-wrap items-center gap-2.5"><span className="h-1.5 w-1.5 rounded-full animate-pulse-dot" style={{ background: UP, boxShadow: `0 0 8px ${UP}` }} /><span className="text-[12.5px] font-semibold tracking-[.18em]" style={{ color: '#9cc1ff' }}>TRADING · VIRT 모의 거래</span></div>
          <h1 className="text-[34.5px] font-bold tracking-tight">거래</h1>
          <p className="mt-2 text-[15px]" style={{ color: INK1 }}>종목을 선택하고 매수·매도를 실행하세요. 실계좌 미연결 상태에서는 VIRT로 안전하게.</p>
        </div>
        ...
        <div className="grid items-start gap-5 grid-cols-1 lg:grid-cols-[minmax(0,260px)_minmax(0,1fr)_minmax(0,300px)]">
          ...
          <div className="flex min-w-0 flex-col gap-[18px]">
            {sel ? <>
              {recent.filter(r => r.stockCode !== sel.symbol).length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {recent.filter(r => r.stockCode !== sel.symbol).slice(0, 6).map(r => (
                    <button key={r.stockCode} onClick={() => selectSymbol(r.stockCode, r.assetType)} className="rounded-full px-2.5 py-1 text-[12.5px] font-semibold" style={{ border: `1px solid ${LINE}`, background: 'var(--ci-card)', color: INK1 }}>{r.stockName}</button>
                  ))}
                </div>
              )}
              ...
              <div style={{ ...mkCard, padding: 0 }}>
                <div className="flex items-center px-1.5" style={{ borderBottom: `1px solid ${LINE}` }}>
                  {tabs.map(([k, l, badge]) => { const on = tab === k; return (
                    <button key={k} onClick={() => setTab(k)} className="relative inline-flex items-center gap-2 px-[18px] py-3.5 text-[15px]" style={{ color: on ? 'var(--ci-ink0)' : INK2, fontWeight: on ? 700 : 500 }}>
                      {l}{badge != null && badge > 0 && <span className="rounded-full px-[7px] py-0.5 text-[12px] font-bold" style={{ background: on ? 'rgba(91,157,255,.22)' : 'var(--ci-line)', color: on ? '#cfe1ff' : INK2 }}>{badge}</span>}
                ...
                {tab === 'order' && <>
                  ...
                  <div className="px-5 pb-4 text-[12px]" style={{ color: INK3 }}>단축키 · <b style={{ color: INK2 }}>B</b> 매수 · <b style={{ color: INK2 }}>S</b> 매도</div>
                </>}
              ...
              <div style={{ ...mkCard, padding: '60px 32px' }} className="text-center text-[14.5px]" >{loading ? <span style={{ color: INK3 }}>종목을 불러오는 중…</span> : <span style={{ color: INK3 }}>종목을 선택해주세요.</span>}</div>
            )}
          </div>
