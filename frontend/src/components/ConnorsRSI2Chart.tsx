...
export default function ConnorsRSI2Chart() {
  ...
  const draw = useCallback(() => {
    ...
    /* ── 시그널: RSI(2) < 5 매수, RSI(2) > 60 매도 ── */
    const signals: { i: number; type: 'buy' | 'sell'; rsiVal: number; price: number }[] = [];
    for (let i = RSI_P + 1; i <= frame && i < N; i++) {
      if (rsi[i] === null || rsi[i - 1] === null) continue;
      if (rsi[i - 1]! >= 5 && rsi[i]! < 5) signals.push({ i, type: 'buy', rsiVal: rsi[i]!, price: closes[i] });
      if (rsi[i - 1]! <= 60 && rsi[i]! > 60) signals.push({ i, type: 'sell', rsiVal: rsi[i]!, price: closes[i] });
    }
