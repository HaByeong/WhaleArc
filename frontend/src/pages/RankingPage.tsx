...
const RankingPage = () => {
  ...
  const getReturnColor = (returnValue: number) => {
    ...
    return isVirt ? 'text-gray-500' : 'text-slate-500';
  };
  ...
  return (
    <div className={`min-h-screen ${isVirt ? 'bg-gray-50' : 'bg-[#060d18] text-white'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* 페이지 헤더 */}
        <div className="mb-8">
          <h1 className={`text-3xl md:text-4xl font-bold ${isVirt ? 'text-whale-dark' : 'text-white'}`}>
            투자 현황
          </h1>
          <p className={`mt-2 text-base ${isVirt ? 'text-gray-500' : 'text-slate-400'}`}>
        ...
        {!loading && !error && rankings.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className={`rounded-xl p-5 border ${isVirt ? 'bg-white border-gray-100' : 'bg-white/[0.02] border-white/[0.06]'}`}>
              <p className={`text-sm mb-1 ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>참여 투자자</p>
              <p className={`text-2xl font-bold ${isVirt ? 'text-whale-dark' : 'text-white'}`}>{stats.totalInvestors}명</p>
            </div>
            <div className={`rounded-xl p-5 border ${isVirt ? 'bg-white border-gray-100' : 'bg-white/[0.02] border-white/[0.06]'}`}>
              <p className={`text-sm mb-1 ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>평균 수익률</p>
              <p className={`text-2xl font-bold ${getReturnColor(stats.avgReturn)}`}>
                {formatReturn(stats.avgReturn)}
              </p>
            </div>
            <div className={`rounded-xl p-5 border ${isVirt ? 'bg-white border-gray-100' : 'bg-white/[0.02] border-white/[0.06]'}`}>
              <p className={`text-sm mb-1 ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>수익 투자자</p>
              <p className="text-2xl font-bold text-red-500">{stats.positiveCount}명</p>
            </div>
            <div className={`rounded-xl p-5 border ${isVirt ? 'bg-white border-gray-100' : 'bg-white/[0.02] border-white/[0.06]'}`}>
              <p className={`text-sm mb-1 ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>손실 투자자</p>
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-whale-light focus:ring-offset-2 ${
                rankingType === filter.key
                  ? isVirt ? 'bg-whale-dark text-white' : 'text-cyan-400 bg-white/10'
                  : isVirt ? 'bg-white text-gray-500 hover:text-gray-700 border border-gray-200' : 'text-slate-500 border border-white/[0.06] hover:bg-white/[0.03]'
              }`}
            >
              {filter.label}
            </button>
        ...
        {/* 투자자 리스트 */}
        {!loading && !error && (
          <div className={`rounded-xl border overflow-hidden ${isVirt ? 'bg-white border-gray-100' : 'bg-white/[0.02] border-white/[0.06]'}`}>
            {/* 테이블 헤더 — 데스크톱만 */}
            <div className={`hidden md:block px-6 py-3.5 border-b ${isVirt ? 'border-gray-100 bg-gray-50/50' : 'border-white/[0.06] bg-white/[0.02]'}`}>
              <div className={`grid grid-cols-12 gap-4 text-xs font-medium uppercase tracking-wider ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>
            ...
            <div className={`divide-y ${isVirt ? 'divide-gray-50' : 'divide-white/[0.04]'}`}>
              {rankings.length === 0 ? (
                <div className="px-6 py-16 text-center">
                  <p className={`font-medium ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>아직 참여한 투자자가 없습니다</p>
                  <p className={`text-sm mt-1 ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>첫 번째 항해를 시작해보세요</p>
                </div>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-shrink-0 w-8 text-center">
                          {ranking.rank === 1 ? (
                            ...
                            <span className={`text-sm font-semibold ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>{ranking.rank}</span>
                          )}
                        </div>
                        {/* 아바타 */}
                        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${
                          ranking.isMyRanking ? 'bg-whale-light' : isVirt ? 'bg-gray-300' : 'bg-slate-600'
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <span className={`text-sm font-medium truncate ${isVirt ? 'text-gray-800' : 'text-white'}`}>{ranking.nickname}</span>
                            {ranking.isMyRanking && (
                          ...
                          {ranking.routeName ? (
                            <div className="flex items-center gap-1 mt-0.5">
                              <span className={`text-xs truncate ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>{ranking.routeName}</span>
                            ...
                            <span className={`text-[11px] ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>항로 미설정</span>
                          )}
                        </div>
                        {/* 수익률 + 자산 */}
                        <div className="flex-shrink-0 text-right">
                          ...
                          <div className={`text-[11px] ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>{formatAmount(ranking.totalValue)}원</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:grid grid-cols-12 gap-4 items-center">
                      ...
                      <div className="col-span-4">
                        <div className="flex items-center space-x-2.5">
                          ...
                          <div>
                            <span className={`text-sm font-medium ${isVirt ? 'text-gray-800' : 'text-white'}`}>{ranking.nickname}</span>
                            {ranking.isMyRanking && (
                      ...
                      <div className="col-span-3">
                        {ranking.routeName ? (
                          <div className="flex items-center gap-1.5">
                            <span className={`text-sm font-medium truncate ${isVirt ? 'text-whale-dark' : 'text-cyan-400'}`}>{ranking.routeName}</span>
                            {ranking.routeStrategyType === 'TURTLE' && (
                              <span className="px-1 py-0.5 text-[8px] font-bold bg-amber-100 text-amber-700 rounded flex-shrink-0">터틀</span>
                            )}
                          </div>
                        ) : (
                          <span className={`text-xs ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>미설정</span>
                        )}
                      </div>
                      <div className="col-span-2 text-right">
                        {ranking.routeReturnRate != null ? (
                          ...
                          <span className={`text-xs ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>-</span>
                        )}
                      </div>
                      <div className="col-span-2 text-right">
                        <span className={`text-sm font-medium ${isVirt ? 'text-gray-700' : 'text-slate-300'}`}>{formatAmount(ranking.totalValue)}</span>
                        <span className={`text-xs ml-0.5 ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>원</span>
                      </div>
                    </div>
                  </div>
                ))
        ...
        {!loading && !error && rankings.length > 0 && totalPages > 1 && (
          <div className="mt-6 flex justify-center items-center space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg border text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${isVirt ? 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50' : 'border-white/[0.06] bg-white/[0.02] text-slate-400 hover:bg-white/[0.03]'}`}
            >
              이전
            </button>
                ...
                return (
                  <button
                    ...
                    className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${
                      currentPage === pageNum
                        ? isVirt ? 'bg-whale-dark text-white' : 'text-cyan-400 bg-white/10'
                        : isVirt ? 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50' : 'text-slate-500 border border-white/[0.06] hover:bg-white/[0.03]'
                    }`}
                  >
                    {pageNum + 1}
                  </button>
        ...
        {!loading && !error && rankings.length > 0 && (
          <p className={`mt-4 text-center text-xs ${isVirt ? 'text-gray-300' : 'text-slate-600'}`}>
      ...
      {selectedRanking && (
        ...
        >
          <div
            className={`rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 ${isVirt ? 'bg-white' : 'bg-[#0c1829] border border-white/[0.06]'}`}
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-sm font-semibold ${isVirt ? 'text-gray-500' : 'text-slate-400'}`}>대표 항로</span>
                </div>

                <div className={`rounded-xl p-4 ${isVirt ? 'bg-gray-50' : 'bg-white/[0.04]'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${isVirt ? 'text-whale-dark' : 'text-white'}`}>{selectedRanking.routeName}</span>
                  ...
                  <div className="flex items-baseline gap-2 mb-3">
                    ...
                    <span className={`text-xs ${isVirt ? 'text-gray-400' : 'text-slate-500'}`}>항로 수익률</span>
                  </div>

                  {/* 전략 정보 */}
                  <div className={`space-y-2.5 pt-3 border-t ${isVirt ? 'border-gray-200' : 'border-white/[0.06]'}`}>
                    <div className="flex justify-between text-sm">
                      <span className={isVirt ? 'text-gray-400' : 'text-slate-500'}>전략 유형</span>
                      <span className={`font-medium ${isVirt ? 'text-gray-700' : 'text-slate-300'}`}>{strategyLabel(selectedRanking.routeStrategyType)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className={isVirt ? 'text-gray-400' : 'text-slate-500'}>총 자산</span>
                      <span className={`font-medium ${isVirt ? 'text-gray-700' : 'text-slate-300'}`}>{formatAmount(selectedRanking.totalValue)}원</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className={isVirt ? 'text-gray-400' : 'text-slate-500'}>포트폴리오 수익률</span>
              ...
              {selectedRanking.routeDescription && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-sm font-semibold ${isVirt ? 'text-gray-500' : 'text-slate-400'}`}>전략 로직</span>
                  </div>
                  <p className={`text-sm leading-relaxed rounded-xl p-4 ${isVirt ? 'text-gray-600 bg-gray-50' : 'text-slate-400 bg-white/[0.04]'}`}>
                    {selectedRanking.routeDescription}
                  </p>
                </div>
              )}
