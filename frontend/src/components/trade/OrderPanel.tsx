...
const OrderTicket = ({ sel, side, setSide, portfolio, usdKrw, rtPrice, notify, onPlaced, isVirt, open }: {
  sel: MarketPrice; side: string; setSide: (s: string) => void; portfolio: Portfolio | null; usdKrw: number;
  rtPrice: number | null; notify: (m: string, t?: 'success' | 'error') => void; onPlaced: () => void; isVirt: boolean; open: boolean;
}) => {
  ...
  // 지정가 기본값 스테일 방지 — 티켓은 패널이 닫혀도 마운트가 유지되고 가격 초기화는 종목 선택 시점뿐이라,
  // 한참 뒤 패널을 열면 옛 가격이 박혀 있었다. 패널을 열 때/지정가로 전환할 때 그 시점 현재가로 리프레시.
  // (열려 있는 동안 사용자가 입력한 지정가는 deps에 가격이 없어 보존됨)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (open && orderMethod === 'LIMIT') setPrice(rtPrice ?? sel.price); }, [open, orderMethod]);
  // 매수/매도 전환 시 수량 초기화
  ...
  return (
    <div>
      ...
      {/* 모드 안내 — virt에선 중립 톤(이미 모의 모드인데 '실계좌 미연결' 경고는 노이즈),
          non-virt에선 경고 톤. 웹 주문은 실계좌 연결 여부와 무관하게 항상 VIRT(페이퍼)로 실행되므로
          '연결되지 않았습니다'(연결해도 뜨는 거짓 문구) 대신 사실 그대로 안내한다. */}
      {isVirt ? (
        <div className="mt-3.5 flex items-center gap-2.5 rounded-lg px-3 py-2.5" style={{ background: 'rgba(91,157,255,.08)', border: '1px solid rgba(91,157,255,.2)' }}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ color: GLOW, flexShrink: 0 }}><circle cx="7" cy="7" r="5.5" stroke="currentColor" strokeWidth="1.4" /><path d="M7 6.5v3.2M7 4.2v.1" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" /></svg>
          <span className="text-[12.5px] leading-snug" style={{ color: INK1 }}>가상 자금으로 연습 중이에요 — 실제 돈이 오가지 않아요.</span>
        </div>
      ) : (
        <div className="mt-3.5 flex items-center gap-2.5 rounded-lg px-3 py-2.5" style={{ background: 'rgba(255,205,120,.08)', border: '1px solid rgba(255,205,120,.18)' }}>
          ...
          <span className="text-[12.5px] leading-snug" style={{ color: '#ffcd78' }}>웹 주문은 <strong>VIRT 모의 거래</strong>로만 실행돼요. 실계좌 매매는 지원하지 않습니다.</span>
        </div>
      )}
...
const OrderPanel = ({ open, onClose, sel, usdKrw, rtPrice, isVirt, side, setSide, notify }: OrderPanelProps) => {
  ...
  // 열림 동안 배경 스크롤 잠금 — 딤 위에서 휠을 굴리면 뒤 페이지가 스크롤되던 것 방지
  useEffect(() => {
    if (!open) return;
      ...
      <aside role="dialog" aria-label="주문 패널" aria-hidden={!open} className="fixed right-0 top-0 z-[96] flex h-full w-full flex-col sm:w-[440px]" style={{ background: 'var(--ci-bg, #0a1024)', borderLeft: `1px solid ${LINE}`, boxShadow: '-20px 0 48px -24px rgba(0,0,0,.6)', transform: open ? 'translateX(0)' : 'translateX(100%)', transition: 'transform .26s cubic-bezier(.4,0,.2,1)' }}>
        {sel ? <>
          ...
          <div className="no-scrollbar flex-1 overflow-y-auto px-5 py-5">
            {tab === 'order' && <>
              <OrderTicket sel={sel} side={side} setSide={setSide} portfolio={portfolio} usdKrw={usdKrw} rtPrice={rtPrice} notify={notify} onPlaced={loadAccount} isVirt={isVirt} open={open} />
