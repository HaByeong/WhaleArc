        ...
        <div className="flex justify-between items-center h-16">
          ...
          >
            <div className="flex items-center gap-1.5">
              {/* 로고가 virt/no-virt 식별 장치: no-virt(고수)는 라이트·다크 모두 항상 캔들차트,
                  virt·랜딩은 친근한 고래. 라이트 배경에선 캔들차트를 어둡게 렌더(lightBg) */}
              <WhaleTailLogo size={40} showNav={showNav} darkNav={showNav && section === 'novirt'} lightBg={!isDark} />
            ...
            <>
              ...
              <nav className="hidden lg:flex items-center space-x-1 flex-1 justify-center h-full" aria-label="주요 네비게이션">
                {[
                  ...
                  { to: `${prefix}/auto-trade`, label: '자동매매' },
              ...
              {isAuthenticated && (
                <div className="hidden lg:flex items-center space-x-2 shrink-0 h-full">
                    ...
                    <button
                      ...
                      aria-label={`테마 변경 (현재: ${isDark ? '다크' : '라이트'})`}
        ...
        {showNav && isMobileMenuOpen && (
          ...
          >
            {[
              ...
              { to: `${prefix}/auto-trade`, label: '자동매매', ariaLabel: '자동매매' },
              ...
              <>
                <div className={`px-4 py-3 mt-2 border-t ${isDarkNav ? 'border-white/[0.06]' : 'border-gray-200'}`}>
                  ...
                  >
                    ...
                    <span className="text-sm">{isDark ? '다크 모드' : '라이트 모드'}</span>
                  </button>
                  {/* 모바일 알림 */}
                  <button
                    onClick={() => { setShowNotifPanel(!showNotifPanel); if (!showNotifPanel) refreshNotifications(); }}
