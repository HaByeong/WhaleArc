...
const ChangeCell = ({ c }: { c: number }) => (
  <div className="text-right">
    <div className="text-[12px]" style={{ color: INK3 }}>변동</div>
    {c === 0 ? <div className="font-mono text-[14px] font-semibold" style={{ color: INK3 }}>—</div>
      : <div className="font-mono text-[14px] font-semibold" style={{ color: c > 0 ? UP : DOWN }}>{c > 0 ? '▲' : '▼'}{Math.abs(c)}</div>}
  </div>
);
const Rrow = ({ t, onRoute }: { t: Row; onRoute: (id: string) => void }) => {
  ...
  return (
    <div className="grid items-center gap-3.5 px-5 py-3.5" style={{ gridTemplateColumns: COLS, background: t.me ? SONAR_DIM : 'transparent', borderTop: `1px solid ${HAIR}`, borderLeft: t.me ? `2px solid ${SONAR}` : '2px solid transparent' }}>
      <span className="text-center font-mono text-[17.5px] font-bold" style={{ color: t.rank <= 3 ? MEDAL[t.rank].c : t.me ? SONAR : INK2 }}>{t.rank}</span>
      <Avatar name={t.name} c={tier.c} />
      <div className="min-w-0">
        <div className="flex items-center gap-2"><span className="truncate text-[15px] font-semibold">{t.name}</span>{t.me && <span className="rounded px-1.5 py-px text-[11px] font-bold text-white" style={{ background: SONAR }}>나</span>}</div>
        <div className="mt-0.5 truncate text-[12.5px] font-semibold" style={{ color: tier.c }}>{tier.label} · {t.strat}</div>
      </div>
      <ChangeCell c={t.change} />
      <div className="text-right"><div className="font-mono text-[18.5px] font-bold" style={{ color: up ? UP : DOWN }}><Tri up={up} />{up ? '+' : ''}{t.ret.toFixed(1)}%</div><div className="mt-0.5 font-mono text-[12px]" style={{ color: INK3 }}>{won(t.total)}</div></div>
      <button onClick={() => onRoute(t.portfolioId)} className="whitespace-nowrap rounded-lg px-3.5 py-2 text-[13.5px] font-semibold" style={t.me ? { border: `1px solid ${HAIR_S}`, background: 'transparent', color: INK1 } : { border: '1px solid rgba(91,157,255,.32)', background: SONAR_DIM, color: SONAR }}>{t.me ? '내 프로필' : '항로 보기'}</button>
    </div>
  );
};
      ...
      {order.map(t => {
        ...
        return (
          <button key={t.rank} onClick={() => onRoute(t.portfolioId)} className="min-w-0 overflow-hidden text-center" style={{ ...panel, padding: '18px 10px', border: `1px solid ${md.border}`, background: md.bg }}>
            ...
            <div className="mt-0.5 text-[12px] font-semibold" style={{ color: tier.c }}>{tier.label}</div>
            ...
            <div className="mt-3 flex items-start justify-center pt-2" style={{ height: htMap[t.rank] - 70, borderRadius: '8px 8px 0 0', background: md.ped }}>
              <span className="font-mono text-[19.5px] font-bold" style={{ color: md.c }}>{t.rank}</span>
            </div>
          </button>
        );
      })}
    ...
    Promise.all([rankingService.getRankings(type, page, PAGE_SIZE).catch(() => null), rankingService.getMyRanking().catch(() => null)])
      .then(([r, m]) => {
        if (r) {
          ...
          const newTotalPages = Math.max(1, r.totalPages || 1);
          setTotalPages(newTotalPages);
          // totalPages가 줄어 현재 page가 범위를 벗어나면 마지막 유효 페이지로 보정(빈 페이지 오표시 방지)
          if (page > newTotalPages - 1) setPage(newTotalPages - 1);
    ...
    <HelmShell active="status" virt={isVirt} userName={userName} session="랭킹 갱신 매일 00:00">
      <div className="mx-auto flex max-w-[1560px] flex-col gap-[18px]">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            <div className="mb-2 flex items-center gap-2.5"><span className="h-1.5 w-1.5 rounded-full animate-pulse-dot" style={{ background: SONAR, boxShadow: `0 0 8px ${SONAR}` }} /><span className="text-[12.5px] font-semibold tracking-[.18em]" style={{ color: SONAR }}>명예의 전당 · 매일 00:00 갱신</span></div>
            <h1 className="text-[28px] font-bold">항해사 랭킹</h1>
            <p className="mt-2 text-[14.5px]" style={{ color: INK1 }}>각 항해사의 대표 포트폴리오 수익률로 매기는 순위. 마음에 드는 항로는 따라가 보세요.</p>
          </div>
          <div className="flex gap-[3px] rounded-[9px] p-[3px]" style={{ background: ABYSS1, border: `1px solid ${HAIR}` }}>
            {PERIODS.map(p => <button key={p.label} onClick={() => changePeriod(p.label)} className="rounded-md px-3.5 py-1.5 text-[13px] font-semibold" style={{ background: period === p.label ? SONAR_DIM : 'transparent', color: period === p.label ? SONAR : INK2 }}>{p.label}</button>)}
          </div>
        </div>

        {loading ? (
          <div style={{ ...panel, padding: '60px 30px' }} className="flex flex-col items-center gap-3"><span className="h-8 w-8 animate-spin rounded-full" style={{ border: `3px solid rgba(91,157,255,.25)`, borderTopColor: SONAR }} /><span className="text-[14px]" style={{ color: INK2 }}>랭킹을 불러오는 중…</span></div>
        ) : error || rows.length === 0 ? (
          <div style={{ ...panel, padding: '60px 30px', textAlign: 'center' }}>
            <div className="mb-1 text-[30px]">{error ? '🧭' : '🌊'}</div>
            <div className="text-[16px] font-semibold">{error ? '랭킹을 불러오지 못했습니다.' : '아직 랭킹 데이터가 없습니다.'}</div>
            <p className="mt-2 text-[14px]" style={{ color: INK3 }}>{error ? '잠시 후 다시 시도해주세요.' : '모의투자로 첫 항해를 시작하면 이곳 순위에 등록됩니다. 가상돈 ₩10,000,000으로 부담 없이 시작해보세요.'}</p>
            {error
          ...
          <>
              ...
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))' }}>
                {[
                  ...
                  <div key={s.l} style={{ ...panel, padding: '14px 18px', borderLeft: `3px solid ${s.a}` }}>
                    <div className="text-[12px] tracking-[.08em]" style={{ color: INK2 }}>{s.l}</div>
                    <div className="mt-1 font-mono text-[21.5px] font-bold" style={{ color: s.c }}>{s.v}</div>
                  </div>
                ))}
              </div>
            )}
              ...
              <div style={{ ...panel, padding: '18px 22px', background: 'linear-gradient(135deg, rgba(91,157,255,.12), rgba(91,157,255,.04))', border: '1px solid rgba(91,157,255,.32)' }}>
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="text-center"><div className="text-[11.5px] tracking-[.1em]" style={{ color: INK2 }}>내 순위</div><div className="font-mono text-[39px] font-bold leading-none">#{my.currentRank}</div></div>
                    <Avatar name={myEntry?.name || userName} c={myTier.c} size={44} />
                    <div><div className="text-[16px] font-bold">{myEntry?.name || userName}</div><div className="mt-0.5 text-[13px] font-semibold" style={{ color: myTier.c }}>{myTier.label} · {myEntry?.strat || '대표 항로 없음'}</div></div>
                  </div>
                  <div className="flex flex-wrap items-center gap-7">
                    <div className="text-right"><div className="text-[12px]" style={{ color: INK3 }}>대표 수익률</div><div className="font-mono text-[28px] font-bold" style={{ color: my.totalReturn >= 0 ? UP : DOWN }}><Tri up={my.totalReturn >= 0} />{my.totalReturn >= 0 ? '+' : ''}{my.totalReturn.toFixed(2)}%</div></div>
                    <div className="text-right"><div className="text-[12px]" style={{ color: INK3 }}>순위 변동</div><div className="font-mono text-[17.5px] font-semibold" style={{ color: myChange > 0 ? UP : myChange < 0 ? DOWN : INK2 }}>{myChange === 0 ? '—' : `${myChange > 0 ? '▲' : '▼'}${Math.abs(myChange)}`}</div></div>
                    {topPct != null && <div className="text-right"><div className="text-[12px]" style={{ color: INK3 }}>상위</div><div className="font-mono text-[17.5px] font-semibold">{topPct}%</div></div>}
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ ...panel, padding: 0 }}>
              <div className="flex flex-wrap items-center justify-between gap-3 px-[22px] py-[18px]" style={{ borderBottom: `1px solid ${HAIR}` }}>
                <div><div className="mb-1 text-[11.5px] font-semibold tracking-[.2em]" style={{ color: SONAR }}>LEADERBOARD</div><h3 className="text-[16.5px] font-bold">전체 랭킹 <span className="font-mono text-[13px] font-medium" style={{ color: INK3 }}>{totalCount}명</span></h3></div>
                <span className="font-mono text-[12.5px]" style={{ color: INK3 }}>{period} 수익률 기준</span>
              </div>
              {/* 좁은 화면에서는 고정 6열 테이블을 가로 스크롤로(페이지 가로 넘침 방지) */}
              <div className="overflow-x-auto">
                <div className="min-w-[560px]">
                  <div className="grid gap-3.5 px-5 py-2.5 text-[11.5px] font-semibold uppercase tracking-[.1em]" style={{ gridTemplateColumns: COLS, color: INK3 }}>
              ...
              {totalPages > 1 && (
                <div className="flex flex-wrap items-center justify-center gap-1.5 px-5 py-4" style={{ borderTop: `1px solid ${HAIR}` }}>
                  <button disabled={page === 0} onClick={() => setPage(p => Math.max(0, p - 1))} className="rounded-lg px-3 py-1.5 text-[13.5px] font-semibold disabled:opacity-35" style={{ border: `1px solid ${HAIR_S}`, background: 'transparent', color: INK1 }}>← 이전</button>
                  {pageWindow(page, totalPages).map(n => (
                    <button key={n} onClick={() => setPage(n)} className="min-w-[34px] rounded-lg px-2.5 py-1.5 font-mono text-[13.5px] font-bold" style={n === page ? { background: SONAR, color: '#fff', border: `1px solid ${SONAR}` } : { border: `1px solid ${HAIR}`, background: 'transparent', color: INK1 }}>{n + 1}</button>
                  ))}
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} className="rounded-lg px-3 py-1.5 text-[13.5px] font-semibold disabled:opacity-35" style={{ border: `1px solid ${HAIR_S}`, background: 'transparent', color: INK1 }}>다음 →</button>
                  <span className="ml-2 font-mono text-[12.5px]" style={{ color: INK3 }}>{page + 1} / {totalPages}</span>
                </div>
              )}
            </div>
          </>
        ...
        <footer className="mt-2 flex flex-col gap-1 pt-5 sm:flex-row sm:flex-wrap sm:justify-between sm:gap-3" style={{ borderTop: `1px solid ${HAIR}` }}>
          <span className="font-mono text-[12.5px]" style={{ color: INK3 }}>© 2026 WHALEARC · 랭킹은 대표 포트폴리오 수익률 기준 · 투자 권유가 아닙니다.</span>
