    ...
    <div onClick={() => !busy && onClose()} className="fixed inset-0 z-[120] flex items-start justify-center overflow-y-auto px-6 py-12"
      ...
      <div onClick={e => e.stopPropagation()} className="relative w-full max-w-[440px] rounded-[18px] overflow-hidden"
        ...
        <div style={{ padding: '20px 22px 16px', borderBottom: `1px solid ${LINE}` }}>
          ...
          <h2 className="mt-3 text-[21.5px] font-bold tracking-tight" style={{ color: INK0 }}>{c.title}</h2>
          <p className="mt-2 text-[13.5px] leading-relaxed" style={{ color: INK2 }}>
          ...
          <p className="mt-2 rounded-lg px-2.5 py-1.5 text-[12.5px] leading-snug" style={{ background: 'var(--ci-card)', border: `1px solid ${LINE}`, color: INK2 }}>
        ...
        <div style={{ padding: '16px 22px' }}>
          <div className="flex items-center justify-between">
            <label className="text-[12.5px] font-semibold tracking-[.04em]" style={{ color: INK2 }}>지금 마음은 어떤가요? <span style={{ color: INK3 }}>(선택)</span></label>
            <span className="text-[11.5px] tabular-nums" style={{ color: INK3 }}>{note.length}/200</span>
          </div>
          <textarea value={note} onChange={e => setNote(e.target.value.slice(0, 200))} rows={2}
            placeholder={c.placeholder}
            className="mt-1.5 w-full resize-none rounded-[10px] px-3 py-2.5 text-[14px] outline-none"
            style={{ border: `1px solid ${LINE}`, background: 'var(--ci-raised)', color: INK0 }} />

          <div className="mt-3 flex items-center justify-between">
            <span className="text-[12.5px] font-semibold" style={{ color: INK2 }}>감정 강도</span>
        ...
        <div style={{ padding: '4px 22px 18px' }}>
          <div className="grid grid-cols-2 gap-2.5">
            <button onClick={() => fire('FOLLOW_IMPULSE')} disabled={busy}
              className="rounded-[11px] py-3 text-[14.5px] font-bold disabled:opacity-50"
            ...
            <button onClick={() => fire('FOLLOW_RULE')} disabled={busy}
              className="rounded-[11px] py-3 text-[14.5px] font-bold text-white disabled:opacity-50"
              style={{ background: 'linear-gradient(180deg, var(--ci-sonar), #2f6fe0)', border: '1px solid rgba(140,190,255,.4)' }}>
              {c.rule}
            </button>
          </div>
          <p className="mt-3 text-center text-[12px]" style={{ color: INK3 }}>
