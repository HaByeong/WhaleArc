...
const ProductModal = ({ p, purchased, isVirt, onClose, onStart, onCancel, prefix }: { p: QuantProduct; purchased: boolean; isVirt: boolean; onClose: () => void; onStart: () => void; onCancel: () => void; prefix: string }) => {
  ...
  const presetId = presetFor(p); // 학습 전략 → 백테스트 빌더 프리셋(있으면 자동 선택)
  const goBacktest = () => navigate(presetId ? `${prefix}/strategy?strategy=${presetId}` : `${prefix}/strategy`);
  return (
    <div onClick={onClose} className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto px-6 py-12" style={{ background: 'rgba(6,11,31,.72)', backdropFilter: 'blur(6px)', animation: 'backdrop-in .2s ease' }}>
      <div onClick={e => e.stopPropagation()} className="relative w-full max-w-[680px] rounded-[18px]" style={{ background: 'var(--ci-overlay)', border: `1px solid ${HAIR_S}`, boxShadow: 'var(--ci-panel-shadow)', animation: 'modal-in .25s cubic-bezier(.2,.8,.2,1)' }}>
        ...
        <footer className="mt-4 flex flex-wrap items-center justify-between gap-3.5 px-7 pb-6 pt-[18px]" style={{ borderTop: `1px solid ${HAIR}` }}>
          <span className="text-[12px]" style={{ color: INK2 }}>{purchased ? '이 전략으로 항해 중입니다.' : '준비됐다면 직접 항해를 시작해보세요.'}</span>
          <div className="flex gap-2.5">
            <button onClick={goBacktest} title={presetId ? '이 전략이 세팅된 채로 백테스트 화면이 열려요' : '백테스트 화면으로 이동'} className="rounded-lg px-4 py-2.5 text-[13px] font-semibold" style={{ border: `1px solid ${HAIR_S}`, background: CARD, color: INK1 }}>{presetId ? '이 전략 백테스트 →' : '백테스트로 검증'}</button>
            {purchased
              ...
              : isVirt
                ? <button onClick={onStart} className="inline-flex items-center gap-2 rounded-[10px] px-[18px] py-2.5 text-[13px] font-bold text-white" style={{ border: '1px solid rgba(140,190,255,.5)', background: 'linear-gradient(180deg,#4d8aff 0%,#2c6fe6 62%,#2257c8 100%)', boxShadow: '0 12px 26px -12px rgba(44,111,230,.7), inset 0 1px 0 rgba(255,255,255,.38)' }}><span className="rounded px-1.5 py-0.5 text-[10px] font-bold" style={{ background: 'rgba(255,255,255,.2)' }}>VIRT</span>항해 시작 →</button>
                : <button onClick={() => navigate('/virt/store')} className="inline-flex items-center gap-2 rounded-[10px] px-[18px] py-2.5 text-[13px] font-bold text-white" style={{ border: '1px solid rgba(140,190,255,.5)', background: 'linear-gradient(180deg,#4d8aff 0%,#2c6fe6 62%,#2257c8 100%)' }}><span className="rounded px-1.5 py-0.5 text-[10px] font-bold" style={{ background: 'rgba(255,255,255,.2)' }}>VIRT</span>모의투자에서 시작</button>}
          </div>
        </footer>
      </div>
    </div>
        ...
        {step === 'amount' ? (
          <>
            <div className="flex flex-col gap-3 p-6 pt-3">
              ...
              <p className="text-[11px]" style={{ color: INK3 }}>입력한 금액으로 전략의 대상 자산을 비중대로 매수해 모의 항해를 시작합니다.</p>
            </div>
          ...
          <>
            <div className="flex flex-col gap-3 p-6 pt-3">
              <p className="m-0 text-[13px] leading-relaxed" style={{ color: INK1 }}>아래 내용으로 <span style={{ color: 'var(--ci-ink0)', fontWeight: 700 }}>모의 항해</span>를 시작합니다. 맞나요?</p>
              <dl className="m-0 grid gap-2.5 rounded-xl px-4 py-3.5 text-[13px]" style={{ background: CARD, border: `1px solid ${HAIR}` }}>
                <div className="flex justify-between"><dt style={{ color: INK2 }}>전략</dt><dd className="m-0 font-semibold">{p.name}</dd></div>
...
const Card = ({ p, purchased, onOpen }: { p: QuantProduct; purchased: boolean; onOpen: () => void }) => {
  const rt = RISK_TAG[p.riskLevel];
  return (
    <div className="flex flex-col" style={panel}>
      ...
      <div className="mt-auto flex items-center justify-between gap-2.5 px-[18px] py-3.5" style={{ borderTop: `1px solid ${HAIR}`, background: CARD }}>
        ...
        {purchased
          ? <span className="inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-[13px] font-bold" style={{ background: 'rgba(239,77,77,.16)', border: '1px solid rgba(239,77,77,.32)', color: UP }}><span className="h-1.5 w-1.5 rounded-full animate-pulse-dot" style={{ background: UP }} />항해 중 ⛵</span>
          : <span className="inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-[13px] font-semibold" style={{ border: '1px solid rgba(91,157,255,.32)', background: SONAR_DIM, color: SONAR }}><span className="rounded px-1.5 py-0.5 text-[10px] font-bold text-white" style={{ background: SONAR }}>VIRT</span>전용</span>}
      </div>
    </div>
  );
};
...
const ConsoleLearnPage = () => {
  ...
  const confirmInvest = async (amount: number) => {
    ...
    try {
      ...
      showToast(`'${invest.name}' 항해를 시작했습니다.`);
      setInvest(null); loadMine();
    } catch (e: any) { showToast(e?.response?.data?.message || '항해 시작에 실패했습니다.'); }
    finally { setBusy(false); }
  };
  // 취소 확인 모달 (window.confirm 대신 스타일 모달)
  ...
  const cancelByProduct = (productId: string) => {
    ...
    setConfirmCancel({ run: async () => {
      try { await quantStoreService.cancelPurchase(pur.id); showToast('항해를 취소했습니다.'); setOpenId(null); loadMine(); }
      catch (e: any) { showToast(e?.response?.data?.message || '취소에 실패했습니다.'); }
    } });
  };
  const cancelByPurchaseId = (purchaseId: string) => {
    setConfirmCancel({ run: async () => {
      try { await quantStoreService.cancelPurchase(purchaseId); showToast('항해를 취소했습니다.'); loadMine(); }
      catch (e: any) { showToast(e?.response?.data?.message || '취소에 실패했습니다.'); }
    } });
  };

  return (
    <HelmShell active="learn" virt={isVirt} userName={userName} session="전략 학습 & 스토어">
      <div className="mx-auto flex max-w-[1560px] flex-col gap-5">
        <div className="flex items-center gap-3">
          <WhaleAvatar size={40} animated />
          <div><h1 className="text-[26px] font-bold tracking-tight">전략 가이드 & 스토어</h1><p className="mt-1.5 text-[13.5px]" style={{ color: INK1 }}>고래 튜터가 전략을 쉽게 설명해드려요. 마음에 들면 VIRT로 항해를 시작해보세요.</p></div>
        </div>
        {/* 항해 현황 (내 보유) */}
        {perf.length > 0 && (
          <div className="flex flex-col gap-3" style={{ ...panel, background: 'linear-gradient(135deg, rgba(91,157,255,.12), rgba(91,157,255,.02) 60%, transparent)', border: '1px solid rgba(91,157,255,.28)', padding: '20px 24px' }}>
            <div className="text-[10.5px] font-semibold tracking-[.18em]" style={{ color: SONAR }}>ACTIVE ROUTES · 항해 중인 전략</div>
            {perf.map(pp => { const up = pp.totalReturnRate >= 0; return (
              <div key={pp.purchaseId} className="flex flex-wrap items-center justify-between gap-4 rounded-xl px-4 py-3" style={{ background: CARD, border: `1px solid ${HAIR}` }}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => cancelByPurchaseId(pp.purchaseId)} className="rounded-[9px] px-[14px] py-2 text-[12.5px] font-semibold" style={{ border: '1px solid rgba(77,138,255,.32)', background: 'rgba(77,138,255,.08)', color: DOWN }}>항해 취소</button>
                </div>
              </div>
            ); })}
          </div>
        ...
        {/* 카드 그리드 */}
        {loading ? <div style={{ ...panel, padding: 48, textAlign: 'center' }}><span className="text-[13px]" style={{ color: INK3 }}>전략 상품을 불러오는 중…</span></div>
          : error ? <div style={{ ...panel, padding: 36, textAlign: 'center' }}><div className="text-[13px]" style={{ color: INK2 }}>{error}</div><button onClick={() => setCat(c => c)} className="mt-3 rounded-lg px-4 py-2 text-[12.5px] font-semibold" style={{ border: `1px solid ${HAIR_S}`, color: SONAR }}>다시 시도</button></div>
            : products.length === 0 ? <div style={{ ...panel, padding: 48, textAlign: 'center' }}><div className="text-[28px]">🧭</div><div className="mt-2 text-[14px] font-semibold">해당 카테고리의 전략이 아직 없어요.</div></div>
              : <div className="grid gap-5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))' }}>
                  {products.map(p => <Card key={p.id} p={p} purchased={purchasedIds.has(p.id)} onOpen={() => setOpenId(p.id)} />)}
                </div>}
      </div>
      {open && <ProductModal p={open} purchased={purchasedIds.has(open.id)} isVirt={isVirt} prefix={prefix} onClose={() => setOpenId(null)} onStart={() => startInvest(open)} onCancel={() => cancelByProduct(open.id)} />}
      ...
      {confirmCancel && (
        <div onClick={() => setConfirmCancel(null)} className="fixed inset-0 z-[120] flex items-center justify-center px-6" style={{ background: 'rgba(6,11,31,.78)', backdropFilter: 'blur(6px)', animation: 'backdrop-in .2s ease' }}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-[380px] rounded-[18px] p-6" style={{ background: 'var(--ci-overlay)', border: `1px solid ${HAIR_S}`, boxShadow: 'var(--ci-panel-shadow)', animation: 'modal-in .25s cubic-bezier(.2,.8,.2,1)' }}>
            <h3 className="text-[16px] font-bold">항해를 취소할까요?</h3>
            <p className="mt-2 text-[13px] leading-relaxed" style={{ color: INK1 }}>이 전략으로 매수한 모의 자산이 정리되고 현금으로 환원됩니다. 되돌릴 수 없어요.</p>
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button onClick={() => { const c = confirmCancel; setConfirmCancel(null); c.run(); }} className="rounded-lg px-4 py-2.5 text-[13px] font-bold text-white" style={{ background: 'linear-gradient(180deg,#e0524f,#c23b38)' }}>항해 취소</button>
            </div>
          </div>
        </div>
      )}
