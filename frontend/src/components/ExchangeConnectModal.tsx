...
const Field = ({ label, value, onChange, type = 'password', placeholder }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder: string }) => (
  <label className="flex flex-col gap-1.5">
    <span className="text-[13px] font-semibold" style={{ color: 'var(--ci-ink2)' }}>{label}</span>
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="rounded-lg px-3 py-2.5 text-[14px] outline-none" style={{ border: '1px solid var(--ci-line)', background: 'var(--ci-card)', color: 'var(--ci-ink0)' }} />
  </label>
);
...
const ExchangeConnectModal = ({ exchangeType, account, onClose, onSaved }: {
  ...
  const save = async () => {
    ...
    } catch (e) { setErr(getErrorMessage(e, '연결에 실패했습니다. API 키를 확인해주세요.')); }
    ...
    <div onClick={onClose} className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto px-6 py-12" style={{ background: 'rgba(6,11,31,.72)', backdropFilter: 'blur(6px)', animation: 'backdrop-in .2s ease' }}>
      <div role="dialog" aria-modal="true" aria-labelledby="exchange-modal-title" onClick={e => e.stopPropagation()} className="relative w-full max-w-[460px] rounded-[18px]" style={{ background: 'var(--ci-overlay)', border: '1px solid var(--ci-line-strong)', boxShadow: 'var(--ci-panel-shadow)', animation: 'modal-in .25s cubic-bezier(.2,.8,.2,1)' }}>
        <div className="wa-force-dark flex items-center justify-between rounded-t-[18px] px-6 py-4 text-white" style={{ background: 'linear-gradient(105deg,#142647 0%,#1d3c7a 52%,#2c6fe6 100%)' }}>
          <h3 id="exchange-modal-title" className="text-[16px] font-bold">{EX_LABEL[exchangeType]} API 연결</h3>
          <button type="button" onClick={onClose} aria-label="닫기" className="flex h-8 w-8 items-center justify-center rounded-lg text-[16px]" style={{ border: '1px solid rgba(255,255,255,.2)', background: 'rgba(255,255,255,.08)' }}><span aria-hidden="true">✕</span></button>
        </div>
        <div className="flex flex-col gap-3 p-6">
          ...
          {isKis && <>
            ...
            <p className="text-[12px]" style={{ color: 'var(--ci-ink3)' }}>한국투자증권 개발자센터(apiportal)에서 발급한 <b>앱키·앱시크릿</b>을 입력하세요(별도 시크릿키 없음). 잔고 조회는 <b>실계좌(실전투자) 키</b> 기준이며 국내주식·미국주식(달러)을 함께 불러옵니다.</p>
          </>}
          ...
          {connected && <div className="rounded-lg px-3 py-2 text-[12.5px]" style={{ background: 'rgba(255,205,120,.08)', border: '1px solid rgba(255,205,120,.18)', color: '#ffcd78' }}>보안상 기존 키는 표시되지 않습니다. 변경하려면 키를 다시 입력하세요.</div>}
          ...
          <p className="text-[12px]" style={{ color: 'var(--ci-ink3)' }}>🔒 API 키는 AES 암호화로 저장되며 읽기 전용 권한만 사용합니다.</p>
        </div>
        <div className="flex gap-2 px-6 py-4" style={{ borderTop: '1px solid var(--ci-line)' }}>
          {connected && <button type="button" onClick={disconnect} className="rounded-lg px-4 py-2.5 text-[14px] font-semibold" style={{ border: '1px solid rgba(239,77,77,.3)', color: UP }}>연결 해제</button>}
          <button type="button" onClick={save} disabled={saving || !form.apiKey || !form.secretKey || (isBitget && !form.appSecret)} className="flex-1 rounded-lg py-2.5 text-[15px] font-bold text-white disabled:opacity-50" style={{ background: `linear-gradient(180deg, ${SONAR}, #2c6fe6)` }}>{saving ? '연결 중…' : connected ? '수정하기' : '연결하기'}</button>
        </div>
      </div>
    </div>
  );
