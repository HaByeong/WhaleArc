...
export const GLOSSARY: Record<string, { title: string; desc: string; example?: string }> = {
  ...
  '모의투자': { title: '모의투자 (VIRT, Paper Trading)', desc: '실제 돈 대신 가상의 돈으로 매매를 연습하는 것입니다. 시세는 실제와 똑같이 움직이지만 손익은 가상이라, 부담 없이 전략을 실험하고 익힐 수 있습니다.', example: 'WhaleArc는 가상돈 ₩1,000만으로 시작합니다' },
  '자동매매': { title: '자동매매 (Auto-Trading)', desc: '전략의 진입·청산 조건을 컴퓨터가 자동으로 감시하다가, 신호가 발생하면 사람 대신 매수·매도를 실행하는 것입니다. 감정 개입 없이 규칙대로 매매됩니다.', example: 'WhaleArc 자동매매는 안전을 위해 모의투자 계좌 전용입니다' },
  ...
  '적립식': { title: '적립식 투자 (DCA, 정액분할매수)', desc: '한 번에 모두 투자하지 않고 매월 일정 금액을 꾸준히 나눠 매수하는 방식입니다. 가격이 쌀 때 더 많이, 비쌀 때 더 적게 사게 되어 평균 매입 단가를 낮추는 효과가 있습니다.', example: '매월 첫 거래일에 50만원씩 자동 매수 → 가격 등락과 무관하게 꾸준히 적립' },
  '배당재투자': { title: '배당 자동 재투자 (DRIP)', desc: '주식·ETF에서 받은 배당금을 현금으로 빼지 않고 즉시 같은 종목을 추가 매수하는 데 쓰는 방식입니다. 복리 효과로 장기 수익을 키웁니다. (미국주식·ETF 한정)', example: 'ON = 배당으로 재매수(Total Return) / OFF = 배당을 현금으로 적립' },
  'BuyHold': { title: 'Buy & Hold (매수 후 보유)', desc: '시작 시점에 매수한 뒤 매매 없이 종료 시점까지 그대로 보유하는 가장 단순한 전략입니다. 전략의 성과를 비교할 때 "기준선(벤치마크)"으로 자주 사용됩니다.', example: '내 전략 수익률이 Buy & Hold보다 높아야 매매할 가치가 있다고 봅니다' },
  '포지션사이징': { title: '포지션 사이징 (Position Sizing)', desc: '한 번 매수할 때 보유 자본의 얼마를 투입할지 정하는 규칙입니다. 전량/자본 비율/고정 금액 중 선택하며, 위험 관리의 핵심 요소입니다.', example: '전량 = 현금 100% / 자본 비율 50% = 절반씩 / 고정 금액 = 매번 100만원씩' },
  '분할매수': { title: '분할 매수 (최대 동시 보유)', desc: '한 번에 다 사지 않고 여러 번 나눠 매수해 동시에 여러 포지션을 보유하도록 하는 설정입니다. 진입 시점을 분산해 매입 단가 위험을 줄입니다.', example: '최대 3회 → 진입 신호가 날 때마다 최대 3번까지 나눠 매수' },
  '시장가': { title: '시장가 주문 (Market Order)', desc: '가격을 정하지 않고 "지금 시세에 바로 사거나 판다"는 주문입니다. 즉시 체결되는 대신, 주문이 몰리거나 호가가 얇으면 생각보다 불리한 가격에 체결될 수 있습니다.', example: '지금 100만원이면 → 시장가 매수는 즉시 100만원 근처에 바로 체결' },
  '지정가': { title: '지정가 주문 (Limit Order)', desc: '"이 가격에만 사거나 판다"고 가격을 미리 정해두는 주문입니다. 원하는 가격에 도달해야 체결되고, 도달하지 않으면 미체결 상태로 대기합니다.', example: '99만원 지정가 매수 → 가격이 99만원 이하로 내려와야 체결, 안 내려오면 대기' },
  '호가': { title: '호가 (Bid·Ask)', desc: '지금 시장에 걸려 있는 매수·매도 주문의 가격과 수량입니다. 가장 높은 매수호가와 가장 낮은 매도호가의 차이를 "스프레드"라 하며, 이 차이가 좁을수록 거래가 유리합니다.', example: '매도호가 100만원 / 매수호가 99.9만원 → 스프레드 1천원' },
  '유동성': { title: '유동성 (Liquidity)', desc: '원하는 때에 원하는 양을, 가격을 크게 흔들지 않고 사고팔 수 있는 정도입니다. 높으면 체결이 빠르고 슬리피지가 작고, 낮으면 주문이 잘 안 체결되거나 가격이 크게 밀립니다.', example: '대형주·메이저 코인 = 유동성 높음 / 소형 종목 = 낮아서 체결 어려움' },
  '거래량': { title: '거래량 (Volume)', desc: '일정 기간 동안 사고팔린 수량입니다. 거래량이 늘면서 가격이 움직이면 신뢰도가 높고, 거래량 없이 오른 가격은 힘이 약한 것으로 봅니다. 봉차트의 V(OHLCV)에 해당합니다.', example: '돌파할 때 거래량 급증 → 진짜 돌파일 가능성↑ (거래량 없는 돌파는 의심)' },
  'ETF': { title: 'ETF (상장지수펀드)', desc: '여러 종목을 하나로 묶어 주식처럼 거래소에서 사고팔 수 있게 만든 상품입니다. ETF 하나만 사도 자동으로 분산투자가 되어, 개별 종목을 고르는 부담이 적습니다.', example: 'S&P500 ETF 1주 = 미국 대표 500개 기업에 조금씩 나눠 투자' },
  '환율': { title: '환율 (Exchange Rate)', desc: '원화와 외화(달러 등)를 교환하는 비율입니다. 미국주식·해외 ETF는 달러로 거래되므로, 주가가 그대로여도 환율이 오르내리면 원화로 환산한 평가금액이 달라집니다.', example: '$100 주식, 환율 1,300→1,400원 → 주가 그대로여도 원화로는 약 +7.7%' },
};
